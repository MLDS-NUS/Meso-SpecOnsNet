...
class AllenCahnPotential(Potential):
    ...
    def _V1(self, u: torch.Tensor) -> torch.Tensor:
        ...
        V1 = ∫ W(u) dx,  W(u) = u^4/(4ε²) - u^2/(2ε²)
        δV1/δu = W'(u) = (u^3 - u)/ε²
        Lyapunov check: u_t = -δV/δu = Δu + (u - u³)/ε²  ✓
        """
        Fu = self.F(u)
        Vu = torch.sum(Fu, dim=-1) * self.dx  # (B,)
        return Vu.unsqueeze(-1)  # (B, 1)
    ...
    def _dV1du_h(self, u_hat: torch.Tensor) -> torch.Tensor:
        ...
        Fpu = self.Fp(u)
        grad_hat = torch.fft.rfft(Fpu, dim=-1)
        return grad_hat
    ...
    def F(self, u: torch.Tensor) -> torch.Tensor:
        """Double-well potential density W(u) = u^4/(4ε²) - u^2/(2ε²)."""
